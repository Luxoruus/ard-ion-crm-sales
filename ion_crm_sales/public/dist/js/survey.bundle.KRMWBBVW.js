(() => {
  ...
  function _o(e, t) {
    ...
    return he(s, e ? { add: $t("add"), set: $t("set"), delete: $t("delete"), clear: $t("clear") } : { add(n) {
      !t && !Ce(n) && !ot(n) && (n = K(n));
      const i = K(this);
  ...
  function Lo(e, t, s = Q) {
    ...
    const J = (I) => {
      if (!(!(d.flags & 1) || !d.dirty && !I))
        if (t) {
          const re = d.run();
          if (n || H || (j ? re.some((ke, G) => rt(ke, te[G])) : rt(re, te))) {
            S && S();
            const ke = dt;
            dt = d;
            try {
              const G = [re, te === es ? void 0 : j && te[0] === es ? [] : te, k];
              te = re, c ? c(t, 3, G) : t(...G);
            } finally {
              dt = ke;
            }
    ...
    return l && l(J), d = new Zr(g), d.scheduler = o ? () => o(J, false) : J, k = (I) => Do(I, false, d), S = d.onStop = () => {
      const I = cs.get(d);
      if (I) {
        if (c)
          c(I, 4);
    ...
    }, t ? r ? J(true) : te = d.run() : o ? o(J.bind(null, true), true) : d.run(), B.pause = d.pause.bind(d), B.resume = d.resume.bind(d), B.stop = B, B;
  }
  ...
  function ir(e) {
    if (!(e.flags & 1)) {
      const t = Wt(e), s = ue[ue.length - 1];
      !s || !(e.flags & 2) && t >= Wt(s) ? ue.push(e) : ue.splice(No(t), 0, e), e.flags |= 1, mn();
    }
  }
  function mn() {
    us || (us = pn.then(vn));
  ...
  function vn(e) {
    try {
      ...
      Fe = -1, ue.length = 0, yn(), us = null, (ue.length || Ct.length) && vn();
    }
  }
  var Te = null;
  var bn = null;
  ...
  function Ht(e, t, s, r, n = false) {
    ...
    if (h != null && h !== c && (ne(h) ? (d[h] = null, k(h) && (g[h] = null)) : ae(h) && (h.value = null)), F(c))
      ...
      if (H || j) {
        const X = () => {
          if (e.f) {
            const B = H ? k(c) ? g[c] : d[c] : c.value;
            n ? P(B) && Zs(B, i) : P(B) ? B.includes(i) || B.push(i) : H ? (d[c] = [i], k(c) && (g[c] = d[c])) : (c.value = [i], e.k && (d[e.k] = c.value));
          } else
  ...
  var Ut = he(/* @__PURE__ */ Object.create(null), { $: (e) => e, $el: (e) => e.vnode.el, $data: (e) => e.data, $props: (e) => e.props, $attrs: (e) => e.attrs, $slots: (e) => e.slots, $refs: (e) => e.refs, $parent: (e) => Ks(e.parent), $root: (e) => Ks(e.root), $host: (e) => e.ce, $emit: (e) => e.emit, $options: (e) => On(e), $forceUpdate: (e) => e.f || (e.f = () => {
    ir(e.update);
  ...
  function ri(e) {
    ...
    const { data: n, computed: i, methods: o, watch: l, provide: c, inject: h, created: d, beforeMount: g, mounted: S, beforeUpdate: k, updated: H, activated: j, deactivated: X, beforeDestroy: B, beforeUnmount: te, destroyed: J, unmounted: I, render: re, renderTracked: ke, renderTriggered: G, errorCaptured: Ee, serverPrefetch: gt, expose: Ne, inheritAttrs: lt, components: mt, directives: Ue, filters: at } = t;
    if (h && ni(h, r, null), o)
      for (const Z in o) {
        const Y = o[Z];
        F(Y) && (r[Z] = Y.bind(s));
    ...
    if (Ws = true, i)
      for (const Z in i) {
        const Y = i[Z], Ve = F(Y) ? Y.bind(s, s) : F(Y.get) ? Y.get.bind(s, s) : De, ct = !F(Y) && F(Y.set) ? Y.set.bind(s) : De, Be = os({ get: Ve, set: ct });
        Object.defineProperty(r, Z, { enumerable: true, configurable: true, get: () => Be.value, set: (pe) => Be.value = pe });
      }
    if (l)
      for (const Z in l)
        Cn(l[Z], r, s, Z);
    ...
    d && xr(d, e, "c");
    function oe(Z, Y) {
      P(Y) ? Y.forEach((Ve) => Z(Ve.bind(s))) : Y && Z(Y.bind(s));
    }
    if (oe(Yo, g), oe(Sn, S), oe(Go, k), oe(Qo, H), oe(Wo, j), oe(zo, X), oe(ei, Ee), oe($o, ke), oe(qo, G), oe(Xo, te), oe(Tn, I), oe(Zo, gt), P(Ne))
      if (Ne.length) {
        const Z = e.exposed || (e.exposed = {});
        Ne.forEach((Y) => {
          Object.defineProperty(Z, Y, { get: () => s[Y], set: (Ve) => s[Y] = Ve, enumerable: true });
        });
      } else
        e.exposed || (e.exposed = {});
  ...
  var oi = { data: _r, props: wr, emits: wr, methods: Ft, computed: Ft, beforeCreate: ce, created: ce, beforeMount: ce, mounted: ce, beforeUpdate: ce, updated: ce, beforeDestroy: ce, beforeUnmount: ce, destroyed: ce, unmounted: ce, activated: ce, deactivated: ce, errorCaptured: ce, serverPrefetch: ce, components: Ft, directives: Ft, watch: li, provide: _r, inject: ii };
  function _r(e, t) {
    return t ? e ? function() {
      return he(F(e) ? e.call(this, this) : e, F(t) ? t.call(this, this) : t);
    } : t : e;
  }
  function ii(e, t) {
    return Ft(zs(e), zs(t));
  ...
  function wr(e, t) {
    return e ? P(e) && P(t) ? [.../* @__PURE__ */ new Set([...e, ...t])] : he(/* @__PURE__ */ Object.create(null), br(e), br(t != null ? t : {})) : t;
  }
  function li(e, t) {
    if (!e)
      return t;
  ...
  function ui(e, t) {
    if (de) {
      ...
      r === s && (s = de.provides = Object.create(r)), s[e] = t;
    }
  }
  ...
  function In(e, t, s = false) {
    ...
    if (P(i))
      ...
      for (const d in i) {
        const g = nt(d);
        if (Sr(g)) {
          const S = i[d], k = o[g] = P(S) || F(S) ? { type: S } : he({}, S), H = k.type;
          let j = false, X = true;
          if (P(H))
            for (let B = 0; B < H.length; ++B) {
              const te = H[B], J = F(te) && te.name;
              if (J === "Boolean") {
                j = true;
                break;
              } else
  ...
  function vi(e, t) {
    ...
    const { insert: r, remove: n, patchProp: i, createElement: o, createText: l, createComment: c, setText: h, setElementText: d, parentNode: g, nextSibling: S, setScopeId: k = De, insertStaticContent: H } = e, j = (a, u, p, v = null, m = null, y = null, T = void 0, w = null, _ = !!u.dynamicChildren) => {
      if (a === u)
        return;
      a && !Rt(a, u) && (v = x(a), pe(a, m, y, true), a = null), u.patchFlag === -2 && (_ = false, u.dynamicChildren = null);
      ...
      if (_ = a.el = o(a.type, y, A && A.is, A), C & 8 ? d(_, a.children) : C & 16 && Ee(a.children, _, null, v, m, Rs(a, y), T, w), R && ut(a, null, v, "created"), G(_, a, a.scopeId, T, v), A) {
        for (const q in A)
      ...
      const V = bi(m, M);
      V && M.beforeEnter(_), r(_, u, p), ((b = A && A.onVnodeMounted) || V || R) && be(() => {
        b && Re(b, v, a), V && M.enter(_), R && ut(a, null, v, "mounted");
      }, m);
    }, G = (a, u, p, v, m) => {
      if (p && k(a, p), v)
      ...
      if (m) {
        ...
        if (u === y || Vn(y.type) && (y.ssContent === u || y.ssFallback === u)) {
          const T = m.vnode;
          G(a, T, T.scopeId, T.slotScopeIds, m.parent);
        }
      }
      ...
      let R;
      if (p && ft(p, false), (R = M.onVnodeBeforeUpdate) && Re(R, p, u, a), A && ut(u, a, p, "beforeUpdate"), p && ft(p, true), (C.innerHTML && M.innerHTML == null || C.textContent && M.textContent == null) && d(w, ""), b ? Ne(a.dynamicChildren, b, w, p, v, Rs(u, m), y) : T || Y(a, u, w, null, p, v, Rs(u, m), y, false), _ > 0) {
        if (_ & 16)
          lt(w, C, M, p, m);
        else if (_ & 2 && C.class !== M.class && i(w, "class", null, M.class, m), _ & 4 && i(w, "style", C.style, M.style, m), _ & 8) {
          const V = u.dynamicProps;
          for (let q = 0; q < V.length; q++) {
            const z = V[q], ge = C[z], me = M[z];
            (me !== ge || z === "value") && i(w, z, ge, me, m, p);
          }
        }
      ...
      for (let w = 0; w < u.length; w++) {
        const _ = a[w], b = u[w], A = _.el && (_.type === fe || !Rt(_, b) || _.shapeFlag & 198) ? g(_.el) : p;
      ...
      R && (w = w ? w.concat(R) : R), a == null ? (r(b, p, v), r(A, p, v), Ee(u.children || [], p, A, m, y, T, w, _)) : C > 0 && C & 64 && M && a.dynamicChildren ? (Ne(a.dynamicChildren, M, p, m, y, T, w), (u.key != null || m && u === m.subTree) && Dn(a, u, true)) : Y(a, u, p, A, m, y, T, w, _);
      ...
      if (_n(a) && (w.ctx.renderer = D), Ui(w, false, T), w.asyncDep) {
        if (m && m.registerDep(w, oe, T), !a.el) {
          ...
          B(null, _, u, p), a.placeholder = _.el;
        }
      } else
        oe(w, a, u, p, m, y, T);
    }, Zt = (a, u, p) => {
      ...
      const w = () => {
        if (a.isMounted) {
          let { next: C, bu: M, u: R, parent: V, vnode: q } = a;
          {
            const Ae = Ln(a);
            if (Ae) {
          ...
          let z = C, ge;
          ft(a, false), C ? (C.el = q.el, Z(a, C, T)) : C = q, M && ts(M), (ge = C.props && C.props.onVnodeBeforeUpdate) && Re(ge, V, C, q), ft(a, true);
          const me = Cr(a), Me = a.subTree;
          a.subTree = me, j(Me, me, g(Me.el), x(Me), a, m, y), C.el = me.el, z === null && Mi(a, me.el), R && be(R, m), (ge = C.props && C.props.onVnodeUpdated) && be(() => Re(ge, V, C, q), m);
        } else {
          let C;
          const { el: M, props: R } = u, { bm: V, m: q, parent: z, root: ge, type: me } = a, Me = Nt(u);
          ft(a, false), V && ts(V), !Me && (C = R && R.onVnodeBeforeMount) && Re(C, z, u), ft(a, true);
          {
          ...
          (u.shapeFlag & 256 || z && Nt(z.vnode) && z.vnode.shapeFlag & 256) && a.a && be(a.a, m), a.isMounted = true, u = p = v = null;
        }
      };
      a.scope.on();
      ...
      for (; b <= C && b <= M; ) {
        const R = a[b], V = u[b] = _ ? et(u[b]) : je(u[b]);
        if (Rt(R, V))
          j(R, V, p, null, m, y, T, w, _);
        else
          break;
        b++;
      }
      for (; b <= C && b <= M; ) {
        const R = a[C], V = u[M] = _ ? et(u[M]) : je(u[M]);
        if (Rt(R, V))
          j(R, V, p, null, m, y, T, w, _);
        else
          break;
      ...
      if (b > C) {
        if (b <= M) {
          const R = M + 1, V = R < A ? u[R].el : v;
          for (; b <= M; )
            j(null, u[b] = _ ? et(u[b]) : je(u[b]), p, V, m, y, T, w, _), b++;
        }
      } else if (b > M)
        for (; b <= C; )
          pe(a[b], m, y, true), b++;
      else {
        const R = b, V = b, q = /* @__PURE__ */ new Map();
        for (b = V; b <= M; b++) {
        ...
        for (b = R; b <= C; b++) {
          ...
          if (ve.key != null)
            ...
            for (z = V; z <= M; z++)
              if (Pt[z - V] === 0 && Rt(ve, u[z])) {
                Pe = z;
                break;
              }
          Pe === void 0 ? pe(ve, m, y, true) : (Pt[Pe - V] = b + 1, Pe >= Ae ? Ae = Pe : Me = true, j(ve, u[Pe], p, null, m, y, T, w, _), ge++);
        }
        const fr = Me ? xi(Pt) : wt;
        for (z = fr.length - 1, b = me - 1; b >= 0; b--) {
          const ve = V + b, Pe = u[ve], dr = u[ve + 1], hr = ve + 1 < A ? dr.el || dr.placeholder : v;
          Pt[b] === 0 ? j(null, Pe, p, hr, m, y, T, w, _) : Me && (z < 0 || b !== fr[z] ? Be(Pe, p, hr, 2) : z--);
        }
      }
    }, Be = (a, u, p, v, m = null) => {
      ...
      if (v !== 2 && b & 1 && w)
        if (v === 0)
          ...
          const { leave: C, delayLeave: M, afterLeave: R } = w, V = () => {
      ...
      let z;
      if (q && (z = T && T.onVnodeBeforeUnmount) && Re(z, u, a), A & 6)
        Os(a.component, p, v);
      else {
        if (A & 128) {
        ...
        V && ut(a, null, u, "beforeUnmount"), A & 64 ? a.type.remove(a, u, p, D, v) : b && !b.hasOnce && (y !== fe || C > 0 && C & 64) ? qe(b, u, p, false, true) : (y === fe && C & 384 || !m && A & 16) && qe(_, u, p), v && At(a);
      }
      (q && (z = T && T.onVnodeUnmounted) || V) && be(() => {
        z && Re(z, u, a), V && ut(a, null, u, "unmounted");
      }, p);
    }, At = (a) => {
      ...
      }), m.stop(), y && (y.flags |= 8, pe(T, a, u, p)), w && be(w, u), be(() => {
        a.isUnmounted = true;
    ...
    let O = false;
    const L = (a, u, p) => {
      a == null ? u._vnode && pe(u._vnode, null, null, true) : j(u._vnode || null, a, u, null, null, null, p), u._vnode = a, O || (O = true, vr(), yn(), O = false);
    }, D = { p: j, um: pe, m: Be, r: At, mt: at, mc: Ee, pc: Y, pbc: Ne, n: x, o: e };
  ...
  function Hn(e, t, s = Q) {
    const { immediate: r, deep: n, flush: i, once: o } = s, l = he({}, s), c = t && r || !t && i !== "post";
    let h;
    if (Jt) {
      if (i === "sync") {
        const k = wi();
  ...
  function Un(e, t, s = false) {
    ...
    return !i && !l ? (ee(e) && r.set(e, null), null) : (P(i) ? i.forEach((c) => o[c] = null) : he(o, i), ee(e) && r.set(e, o), o);
  }
  function ws(e, t) {
  ...
  function Cr(e) {
    ...
    try {
      if (s.shapeFlag & 4) {
        const I = n || r, re = I;
        B = je(h.call(re, I, d, g, k, S, H)), te = l;
      } else {
        const I = t;
        B = je(I.length > 1 ? I(g, { attrs: l, slots: o, emit: c }) : I(g, null)), te = t.props ? l : Oi(l);
      }
    } catch (I) {
      Vt.length = 0, xs(I, e, 1), B = Ye(it);
    }
    let J = B;
    if (te && j !== false) {
      const I = Object.keys(te), { shapeFlag: re } = J;
      I.length && re & 7 && (i && I.some(Xs) && (te = ki(te, i)), J = Et(J, te, false, true));
    }
    return s.dirs && (J = Et(J, null, false, true), J.dirs = J.dirs ? J.dirs.concat(s.dirs) : s.dirs), s.transition && lr(J, s.transition), B = J, fs(X), B;
  }
  var Oi = (e) => {
    let t;
    for (const s in e)
  ...
  function f(e, t = null, s = null, r = 0, n = null, i = e === fe ? 0 : 1, o = false, l = false) {
  ...
  function Ri(e, t = null, s = null, r = 0, n = null, i = false) {
    ...
    if (Wi(e) && (e = e.__vccOpts), t) {
      ...
      l && !ne(l) && (t.class = Se(l)), ee(c) && (or(c) && !P(c) && (c = he({}, c)), t.style = vs(c));
    }
    const o = ne(e) ? 1 : Vn(e) ? 128 : Ko(e) ? 64 : ee(e) ? 4 : F(e) ? 2 : 0;
    return f(e, t, s, r, n, o, i, true);
  }
  function Fi(e) {
    return e ? or(e) || An(e) ? he({}, e) : e : null;
  }
  ...
  function je(e) {
    return e == null || typeof e == "boolean" ? Ye(it) : P(e) ? Ye(fe, null, e.slice()) : Kn(e) ? et(e) : Ye(Ss, null, String(e));
  }
  function et(e) {
  ...
  var Ys;
  {
    ...
    hs = t("__VUE_INSTANCE_SETTERS__", (s) => de = s), Ys = t("__VUE_SSR_SETTERS__", (s) => Jt = s);
  }
  var Xt = (e) => {
    const t = de;
      ...
      try {
        ...
        if (O.message === "Logged In") {
          if ((await Ne(o.username)).includes("Pre-Sale"))
            return t.value = true, s.value = { name: o.username, full_name: "Technical Assessment User", roles: ["Technical Assessment"] }, await Promise.all([Ue(), pe()]), G("Login successful!"), { success: true };
          {
        ...
        return x.code === 401 ? G("Unauthorized access", "error") : G(J("loginError"), "error"), { success: false, error: x.message };
      } finally {
        e.value = false;
      }
    }
    const mt = async () => {
      await fetch("/api/method/logout", { method: "POST", credentials: "include" }), t.value = false, s.value = null, c.value = null, h.value = null, d.value = [], g.value = null, Object.keys(S).forEach((x) => delete S[x]), B && clearInterval(B), G("Logged out successfully");
    }, Ue = async () => {
      l.value = [];
      ...
      try {
        ...
        console.error("Error fetching survey templates:", x), G("Error loading survey templates", "error");
      }
    }, Zt = async (x) => {
      if (confirm(`Are you sure you want to survey "${x.title}"? You will be assigned as the surveyor for this opportunity.`)) {
        ...
        try {
          ...
          c.value = x, await at(), await Ue(), G("You have been assigned as the surveyor for this opportunity");
        } catch (O) {
...
`).filter((u) => u.trim() !== "") : void 0 })) }, Object.keys(S).forEach((a) => delete S[a]), yt.forEach((a) => {
            S[a.field_name] = qe(a);
          }), Y(), r.value = "fill-survey", G(`Template "${D.template_name}" selected successfully`);
        } catch (O) {
    ...
    }, Z = (x) => ({ Data: "Text", "Long Text": "Text", Select: "Multi-choice", Rating: "Rating", Question: "Yes/No" })[x] || "Text", Y = () => {
      ...
      return h.value.questions.forEach((O) => {
        O.required && (!S[O.name] || S[O.name] === "") && (k[O.name] = J("validationError"), x = false);
      }), x;
    }, ct = async (x = false) => {
      var _a2;
      e.value = !x;
      try {
        const O = { opportunity: (_a2 = c.value) == null ? void 0 : _a2.name, template: g.value, answers: __spreadValues({}, S) }, L = await fetch("/api/method/ion_crm_sales.api.save_survey", { method: "POST", headers: { "Content-Type": "application/json", "X-Frappe-CSRF-Token": window.csrf_token }, credentials: "include", body: JSON.stringify({ survey_data: O }) }), D = await L.json();
        if (!L.ok || (D == null ? void 0 : D.exc_type))
          throw new Error((D == null ? void 0 : D.message) || "Error saving draft");
        x || G(J("saveSuccess"));
      } catch (O) {
        x || G(O.message || "Error saving draft", "error");
      } finally {
        e.value = false;
      }
    }, Be = async () => {
      if (!Ve()) {
        G(J("validationError"), "error");
        return;
      }
      G("Survey is valid. Please use Save Draft to save your progress.");
    }, pe = async () => {
      ...
      if (x.status === "In Progress") {
        ...
        try {
          ...
          if (O.ok) {
            const L = await O.json();
            c.value = { name: L.data.name, title: L.data.title, doctype: x.doctype, customer_name: L.data.customer_name, status: L.data.status, workflow_state: L.data.workflow_state }, await at(), r.value = "fill-survey", G(`Continuing survey for: ${x.title}`);
          } else
        ...
        G(`Viewing submitted survey for: ${x.title}`);
...
* @license MIT
**/
//# sourceMappingURL=survey.bundle.KRMWBBVW.js.map
